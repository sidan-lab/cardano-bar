...
import { RSCodeBuilder } from "../codeBuilder/rs";

/**
 * Rust-specific blueprint parser
 * Handles all Rust code generation logic independently
 */
export class RustBlueprintParser {
  preamble: Preamble;
  ...
  generateTypes() {
    if (this.customs.size > 0) {
      this.customs.forEach((custom) => {
        ...
        const typeCode = this.codeBuilder.exportType(
          customTitle,
          this.titleCodeMap[customTitle],
          this.titleCodeMap
        );
        // Only push non-empty type definitions (skip circular aliases that return empty string)
        if (typeCode && typeCode.trim()) {
          this.typeSnippet.push(typeCode);
        }
      });
    }
    return this;
  ...
  generateImports(blueprintPath: string) {
    if (this.imports.size > 0) {
      this.importSnippet = [
        this.codeBuilder.importBlueprint("blueprint", blueprintPath),
        this.codeBuilder.importPackage(
          Array.from(this.imports),
          this.codeBuilder.typePackage()
        ),
      ];
    }
    return this;
  ...
  private getDefinitionCode(title_or_key: string, def: Definition): string {
    ...
    if (title && def.anyOf) {
      ...
      return this.codeBuilder.getConstrCode(constructors);
    }
  ...
  private handleConstructor = (constr: Definition, currentTitle: string) => {
    ...
    return this.codeBuilder.combineVariantWithFields!(constructorTitle!, constructorCode);
  };
  ...
  /**
   * Maps a definition to its title.
   * @param def The definition to map.
   * @param title The title to associate with the definition.
   */
